-module(auctions_create_auction_handler).
-moduledoc """
Exposes the following operation IDs:

- `POST` to `/auctions`, OperationId: `create_auction`:
Create an auction.
Create an auction. Note that the auction models are restricted to be a closed bid auction either as a [First price sealed bid auction](https://en.wikipedia.org/wiki/First-price_sealed-bid_auction) or a [Vickrey auction](https://en.wikipedia.org/wiki/Vickrey_auction). It can also be a [Timed ascending auction also known as an English auction](https://en.wikipedia.org/wiki/English_auction).
...
handle_type_accepted(Req, #state{operation_id = OperationID,
    ...
    {Res, Req1, Context1} = Handler(createAuction, OperationID, Req, Context),
