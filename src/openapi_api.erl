...
validate_response('add_bid', 200, Body, ValidatorState) ->
    validate_response_body('', '', Body, ValidatorState);
validate_response('add_bid', 400, Body, ValidatorState) ->
    validate_response_body('Errors', 'Errors', Body, ValidatorState);
validate_response('add_bid', 404, Body, ValidatorState) ->
    validate_response_body('', '', Body, ValidatorState);
validate_response('get_auction', 200, Body, ValidatorState) ->
    validate_response_body('AuctionModel', 'AuctionModel', Body, ValidatorState);
validate_response('get_auctions', 200, Body, ValidatorState) ->
    validate_response_body('list', 'AuctionModel', Body, ValidatorState);
validate_response(_OperationID, _Code, _Body, _ValidatorState) ->
    ok.

%%%
-spec request_params(OperationID :: operation_id()) -> [Param :: request_param()].
request_params('create_auction') ->
    [
        'x-api-version',
        'CreateAuctionModel'
    ];
request_params('add_bid') ->
    [
        'auctionId',
        'x-api-version',
        'CreateBidModel'
    ];
request_params('get_auction') ->
    [
        'auctionId',
        'x-api-version'
    ];
request_params('get_auctions') ->
    [
        'x-api-version'
    ];
request_params(_) ->
...
request_param_info('create_auction', 'x-api-version') ->
    #{
        source => header,
        rules => [
            {type, binary},
...
request_param_info('create_auction', 'CreateAuctionModel') ->
    #{
        source => body,
        rules => [
            {schema, object, <<"#/components/schemas/CreateAuctionModel">>},
            not_required
        ]
    };
request_param_info('add_bid', 'auctionId') ->
    #{
        source => binding,
        rules => [
            {type, integer},
...
request_param_info('add_bid', 'x-api-version') ->
    #{
        source => header,
        rules => [
            {type, binary},
...
request_param_info('add_bid', 'CreateBidModel') ->
    #{
        source => body,
        rules => [
            {schema, object, <<"#/components/schemas/CreateBidModel">>},
            not_required
        ]
    };
request_param_info('get_auction', 'auctionId') ->
    #{
        source => binding,
        rules => [
            {type, integer},
...
request_param_info('get_auction', 'x-api-version') ->
    #{
        source => header,
        rules => [
            {type, binary},
...
request_param_info('get_auctions', 'x-api-version') ->
    #{
        source => header,
        rules => [
            {type, binary},
