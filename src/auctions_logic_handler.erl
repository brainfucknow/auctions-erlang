...
-type accept_callback() ::
    fun(
        (auctions_api:class(), auctions_api:operation_id(), cowboy_req:req(), context()) -> {
...
-export_type([
    context/0,
    api_key_callback/0,
    accept_callback_return/0,
    provide_callback_return/0,
    accept_callback/0,
    provide_callback/0
]).
...
api_key_callback(OperationID, ApiKey) ->
    ?LOG_ERROR(#{
        what => "Got not implemented api_key_callback request",
...
accept_callback('createAuction', 'create_auction', Req, Context) ->
    ...
    try auction_serialization:decode_auction_req(Body) of
        AuctionReq ->
            ...
            EndsAtMs = calendar:rfc3339_to_system_time(binary_to_list(EndsAtBin), [
                {unit, millisecond}
            ]),
            ...
            {LogicMod, LogicState} =
                case Type of
                    {english, _Min, _Reserve, _Inc} ->
                        ...
                        {english_auction,
                            english_auction:new(#{ends_at => EndsAtMs, options => OptionsBin})};
                    blind ->
        ...
        Class:Reason:Stacktrace ->
            ?LOG_ERROR(#{
                what => "Error processing create_auction",
                class => Class,
                reason => Reason,
...
accept_callback('createBid', 'add_bid', Req, Context) ->
    ...
    try binary_to_integer(AuctionIdBin) of
        AuctionId ->
            ...
            try auction_serialization:decode_bid_req(Body) of
                #{amount := Amount} ->
                    ...
                    Time = calendar:system_time_to_rfc3339(os:system_time(millisecond), [
                    ...
                    case auction_store:add_bid(AuctionId, StoredBid) of
                        ok ->
                            ?LOG_INFO(#{
                                what => "Bid added", auction_id => AuctionId, bid => StoredBid
                            }),
                            RespBid = #{
                                <<"amount">> => Amount,
                                <<"bidder">> => Bidder,
                        ...
                        {error, Reason} ->
                            Req2 = cowboy_req:reply(
                                400, #{}, list_to_binary(io_lib:format("~p", [Reason])), Req1
                            ),
                            {stop, Req2, Context}
                    end
            catch
                Class:Reason:Stacktrace ->
                    ?LOG_ERROR(#{
                        what => "Error processing add_bid inner",
                        class => Class,
                        reason => Reason,
                        stacktrace => Stacktrace
                    }),
        ...
        Class:Reason:Stacktrace ->
            ?LOG_ERROR(#{
                what => "Error processing add_bid outer",
                class => Class,
                reason => Reason,
                stacktrace => Stacktrace
            }),
...
provide_callback('getAuctions', 'get_auction', Req, Context) ->
    ...
    try binary_to_integer(AuctionIdBin) of
        ...
        Class:Reason:Stacktrace ->
            ?LOG_ERROR(#{
                what => "Error processing get_auction",
                class => Class,
                reason => Reason,
                stacktrace => Stacktrace
            }),
