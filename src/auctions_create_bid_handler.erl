...
- `POST` to `/auctions/:auctionId/bids`, OperationId: `add_bid`:
...
allowed_methods(Req, #state{operation_id = 'add_bid'} = State) ->
    {[<<"POST">>], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'add_bid'} = State) ->
    {true, Req, State};
