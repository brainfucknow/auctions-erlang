...
-define(FIRST_AUCTION_REQ,
    <<"{\"id\":1,\"startsAt\":\"2018-01-01T10:00:00.000Z\",\"endsAt\":\"2019-01-01T10:00:00.000Z\",\"title\":\"First auction\", \"currency\":\"VAC\" }">>
).
...
add_auction_duplicate_fails(BaseUrl, Seller1, FirstAuctionReq) ->
    {ok, {{_, 400, _}, _, Body2}} = httpc:request(
        post,
        {
            BaseUrl ++ "/auctions",
            [{"x-jwt-payload", binary_to_list(Seller1)}],
...
add_bid_to_nonexistent_auction_fails(BaseUrl, Buyer1) ->
    {ok, {{_, 404, _}, _, Body3}} = httpc:request(
        post,
        {
